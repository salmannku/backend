...
export class SpeakersController {
  static addSpeaker = catchAsync(async (req: any, res: any) => {
    const result=await SpeakerServices.addSpeaker(req.user,req.body,req?.file,res,sendResponse);
    return result;
  })
  ...
  static getSpeakers = catchAsync(async (req: any, res: any) => {
    ...
    const speakers = await (SpeakersModel as any).paginate(query, options)
  ...
  static getSpeakerById = catchAsync(async (req: any, res: any) => {
    const userId = req?.params?.user_id

    const user = await SpeakersModel.findById(userId)
      .populate({
        path: 'events',
        model: EventsModel,
        select: '_id name',
  ...
  static getAssignedEvents = catchAsync(async (req: any, res: any) => {
    ...
    query.speakers = new ObjectId(userId)
  ...
  static update = catchAsync(async (req: any, res: any) => {
    const result=await SpeakerServices.updateSpeaker(req.params.speaker_id,req.body,req?.file,res,sendResponse);
    return result;
  })
  ...
  static delete = catchAsync(async (req: any, res: any) => {
    ...
    const deleteResponse = await SpeakersModel.findByIdAndDelete(userId)
    ...
    await EventServices.removeSpeaker({
      speaker_id: userId,
    })
  ...
  static changeAccountStatus = catchAsync(async (req: any, res: any) => {
    const userId = req?.params?.speaker_id

    const { status } = req.body

    const updateResponse = await SpeakersModel.findByIdAndUpdate(
      userId,
      {
        status: status,
      },
  ...
  static getProfileSurvey = catchAsync(async (req: any, res: any) => {
    ...
    const profileSurvey = await SpeakerServices.getProfileSurvey({
      user_id: userId,
    })
