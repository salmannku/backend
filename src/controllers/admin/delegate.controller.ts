...
import AdminModel from '../../models/delegates.model'
...
export class DelegateController {
  static addDelegate = catchAsync(async (req: any, res: any) => {
    const result = await DelegateServices.addDelegateService(req.user, req.body, req?.file, res,sendResponse);
    return result;
  })
  ...
  static getDelegateById = catchAsync(async (req: any, res: any) => {
    const userId = req?.params?.user_id

    const user = await DelegatesModel.findById(userId)
      .populate({
        path: 'events',
        model: EventsModel,
        select: '_id name',
  ...
  static getAssignedEvents = catchAsync(async (req: any, res: any) => {
    ...
    query.delegates = new ObjectId(userId)
  ...
  static update = catchAsync(async (req: any, res: any) => {
    const result=await DelegateServices.updateDelegateService(req.params.delegate_id,req.body,req?.file,res,sendResponse);
    return result;
  })
  ...
  static delete = catchAsync(async (req: any, res: any) => {
    ...
    const deleteResponse = await DelegatesModel.findByIdAndDelete(userId)
    ...
    await EventServices.removeDelegate({
      delegate_id: userId,
    })
  ...
  static changeAccountStatus = catchAsync(async (req: any, res: any) => {
    const userId = req?.params?.delegate_id

    const { status } = req.body

    const updateResponse = await DelegatesModel.findByIdAndUpdate(
      userId,
      {
        status: status,
      },
  ...
  static getProfileSurvey = catchAsync(async (req: any, res: any) => {
    ...
    const profileSurvey = await DelegateServices.getProfileSurvey({
      user_id: userId,
    })
